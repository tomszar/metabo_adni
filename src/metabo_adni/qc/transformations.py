...
def imputation(
    ...
    for key in dat_dict:
        ...
        print(
            f"{len(mets_to_impute)} metabolites and {data_points_impute} "
...
def merge(dat_dict: dict[str, pd.DataFrame], platform: str) -> dict[str, pd.DataFrame]:
    ...
    """
    print(
        "=== Merging dataframes and leaving only metabolite columns and "
        + "participants ==="
    )
    dat_copy = dat_dict.copy()
    for key in dat_copy:
        metabo_names = load._get_metabo_col_names(dat_copy[key], key)
        indices = load._get_data_indices(dat_copy[key], platform)
        dat = dat_copy[key].loc[indices, metabo_names]
        dat_copy[key] = dat
    if platform == "p180":
        merge1 = dat_copy["ADNI1-FIA"].merge(
            dat_copy["ADNI1-UPLC"], how="inner", on="RID", suffixes=("_1FIA", "_1UPLC")
        )
        merge2 = dat_copy["ADNI2GO-FIA"].merge(
            dat_copy["ADNI2GO-UPLC"],
            how="inner",
            on="RID",
...
def zscore(dat_dict: dict[str, pd.DataFrame], platform: str) -> dict[str, pd.DataFrame]:
    ...
    for key in dat_dict:
        ...
        indices = load._get_data_indices(dat_dict[key], platform)
        dat = dat_dict[key].loc[indices, metabo_names]
        zscore_dat = dat.apply(stats.zscore, nan_policy="omit")
...
def residualize_metabolites(
    ...
    """
    print(
        "=== Replacing metabolite concentration values for residuals from "
        + "medications ==="
    )
...
def _get_residuals(outcomes, predictors):
    ...
    """
    new_dat = pd.merge(outcomes, predictors, left_on="RID", right_on="RID")
    ...
    for y in Y:
        ...
        while n_not_significants > 0:
            ...
            if not med_names:
                ...
                results = sm.OLS(exog=X[med_names], endog=Y[y]).fit()
                n_significants = sum(results.pvalues < 0.05)
                n_not_significants = sum(results.pvalues > 0.05)
        if n_significants > 0:
